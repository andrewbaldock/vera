# VERA — Production Readiness

**What it would take to run this for real.** [`ARCHITECTURE.md` §9](ARCHITECTURE.md) names the
seams; this expands each into the work behind it, then covers what a demo may skip and a lender's
underwriting tool may not. **Part 1 is work the build was shaped around:** swaps at named
boundaries, not rewrites. **Part 2 is work this build does not touch at all.**
...
[`DESIGN.md` D2](DESIGN.md) rejected every client-side way of guessing. An optional `bbox` is
...
The answer is then a search UI over `pdf.js`'s extracted text, which [`DESIGN.md` D1](DESIGN.md)
...
- **`localStorage` is the data store.** Submissions, done marks and notes live on the device
  ...
  Safari in private browsing throws. A demo's defense, not a server.
