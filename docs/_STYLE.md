...
That sentence belongs in [09-decisions.md](09-decisions.md). If a paragraph starts doing this, move it there and link it. **That is the mechanism that keeps 02 and 03 short:** every place tempted into a sales comparison is a decision that has not been written down yet.
...
These rules are also a skill: [`.claude/skills/depurple/`](../.claude/skills/depurple/SKILL.md).

It does the mechanical half — finding the invented alternatives, the metaphors,
the personified UI, and the counts and cross-references that have gone stale —
