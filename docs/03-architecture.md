...
| Panel sizes | `vera.panels` | **Device** | The issues split, the strip's width, and whether the strip is open. `strip` is `null` until it has actually been dragged, and that null is what keeps the strip a minimap by default — a stored default would switch every reader into the resized behavior on their first visit. |
